...
let Chaincode = class {
  ...
  //BlockCerti Certificates 
  async createCertificate(stub, args) {
    console.log('============= START : createCertificate ===========');
    console.log('##### createCertificate arguments: ' + JSON.stringify(args));

    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = 'certificate' + json['CID'];
    json['docType'] = 'certificate';

    console.log('##### createCertificate certificate: ' + JSON.stringify(json));
  ...
  //  }
    ...
    let userKey = 'User' + json['emailID'];
    let userQuery = await stub.getState(userKey);
    if (!userQuery.toString()) {
      throw new Error('##### createCertificate - Cannot create certificate as the useremail does not exist: ' + json['emailid']);
    }
  ...
  //  if (donationQuery.toString()) {
   ...
   // }
    ...
    await stub.putState(key, Buffer.from(JSON.stringify(json)));
    console.log('============= END : createCertificate ===========');
  }

  /**
   * Retrieves a specfic certificate from CID
   * 
   * @param {*} stub 
   * @param {*} args 
   */
  async queryCertificateFromCID(stub, args) {
    console.log('============= START : queryCertificateFromCID ===========');
    console.log('##### queryCertificateFromCID arguments: ' + JSON.stringify(args));

    // args is passed as a JSON string
    let json = JSON.parse(args);
    let key = 'certificate' + json['CID'];
    console.log('##### queryCertificateFromCID key: ' + key);
    return queryByKey(stub, key);
  }

  /**
   * Retrieves certificates for a specfic User from their EmailID
   * 
   * @param {*} stub 
   * @param {*} args 
   */
  async queryCertificatesFromEmailID(stub, args) {
    console.log('============= START : queryCertificatesFromEmailID ===========');
    console.log('##### queryCertificatesFromEmailID arguments: ' + JSON.stringify(args));

    // args is passed as a JSON string
    let json = JSON.parse(args);
    let queryString = '{"selector": {"docType": "certificate", "emailID": "' + json['emailID'] + '"}}';
    return queryByString(stub, queryString);
  }
   ...
   * @param {*} args 
   */
  async queryAllCertificates(stub, args) {
    console.log('============= START : queryAllCertificates ===========');
    console.log('##### queryAllCertificates arguments: ' + JSON.stringify(args)); 
    let queryString = '{"selector": {"docType": "certificate"}}';
    return queryByString(stub, queryString);
  }
}
